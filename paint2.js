alert("WELCOME TO BASIC PAINT.\nEnjoy drawing on all devices!!.\nThis basic paint project is developed using HTML, CSS, JS.\nThis project enables us to select the colours from the options and do some basic drawings like line drawings, and we can undo and clear the drawings done.");
...
function getCursorPosition(e) {
    ...
    let x = (e.clientX ? e.clientX - rect.left : e.touches[0].clientX - rect.left) * scaleX;
    let y = (e.clientY ? e.clientY - rect.top : e.touches[0].clientY - rect.top) * scaleY;
...
function draw(e) {
    ...
    ctx.beginPath();
    ctx.moveTo(currentLine[currentLine.length - 1][0], currentLine[currentLine.length - 1][1]);
